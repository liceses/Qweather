...
public:
    ...
    // requestCounts — Get per-endpoint request & cache-hit counts / 获取各端点请求和缓存命中计数
    Q_INVOKABLE QVariantMap requestCounts() const;
    Q_INVOKABLE void saveCounts();
    Q_INVOKABLE void loadCounts();
...
private:
    ...
    int m_cacheHitCount[24] = {0};   // Per-endpoint cache-hit counter / 各端点缓存命中计数器
};
